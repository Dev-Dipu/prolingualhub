...
export default function LanguageDropdown() {
    ...
    // Map code to label
    const selected = language === "ru" ? "RUSSIAN" : "ENGLISH";

    const handleSelect = (lang) => {
        let target = "/";
        const currentPath = pathname || "/";
        // Remove trailing slash if present (except root) for consistent matching
        const cleanPath =
            currentPath.length > 1 && currentPath.endsWith("/")
                ? currentPath.slice(0, -1)
                : currentPath;

        // Remove /ru prefix if present to get the "base" path
        const basePath = cleanPath.startsWith("/ru")
            ? cleanPath.replace(/^\/ru/, "")
            : cleanPath;
        // If result is empty string (e.g. /ru -> ""), make it /
        const finalBase = basePath || "/";

        if (lang === "ENGLISH") {
            // English is the root path
            target = finalBase;
        } else if (lang === "RUSSIAN") {
            // Russian is /ru + base path
            // If base is /, target is /ru. Else /ru/base
            if (finalBase === "/") target = "/ru";
            else target = `/ru${finalBase}`;
        }
            ...
            {open && (
                <div className="mt-1.5 w-40 bg-white border border-gray-200 rounded-sm shadow-md">
                    {languages.map((lang) => (
                        ...
                        >
                            <span
                                className={`w-1.5 h-1.5 inline-block ${selected === lang ? "bg-gray-900" : "bg-gray-400"}`}
                            />
